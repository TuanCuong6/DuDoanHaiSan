...
const ManagerEditAreaScreen = ({ navigation, route }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    area_type: area?.area_type || 'oyster',
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await areasAPI.update(area.id, areaData);
      Alert.alert('Thành công', 'Cập nhật khu vực thành công', [
        { text: 'OK', onPress: () => navigation.goBack() }
      ]);
    } catch (error) {
      console.error('Error:', error);
      Alert.alert('Lỗi', 'Không thể cập nhật khu vực');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Sửa khu vực</Text>
        <View style={{ width: 40 }} />
      </View>
...
export default ManagerEditAreaScreen;
