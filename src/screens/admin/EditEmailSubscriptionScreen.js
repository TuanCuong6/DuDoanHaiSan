...
const EditEmailSubscriptionScreen = ({ navigation, route }) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await emailAPI.update(subscription.id, {
        email: email.trim(),
        area_id: selectedArea.id,
        is_active: isActive,
      });
      Alert.alert('Thành công', 'Đã cập nhật đăng ký', [
        { text: 'OK', onPress: () => navigation.goBack() }
      ]);
    } catch (error) {
      Alert.alert('Lỗi', error.response?.data?.message || 'Không thể cập nhật đăng ký');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Sửa đăng ký email</Text>
        <View style={{ width: 40 }} />
      </View>
...
export default EditEmailSubscriptionScreen;
