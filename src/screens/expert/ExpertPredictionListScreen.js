...
const ExpertPredictionListScreen = ({ navigation }) => {
  ...
  const loadUserInfo = async () => {
    try {
      ...
      if (userInfo && userInfo.id) {
        ...
        loadPredictions(false, userInfo.id);
      } else {
  ...
  const loadPredictions = async (isLoadMore = false, userIdParam = null) => {
    ...
    try {
      ...
      const response = await expertPredictionsAPI.getByUser(currentUserId, limit, currentOffset);
      
      if (response.data) {
        // API trả về object với rows
        let newPredictions = [];
        if (Array.isArray(response.data)) {
          newPredictions = response.data;
        } else if (response.data.rows) {
          newPredictions = response.data.rows;
        } else if (response.data.predictions) {
          newPredictions = response.data.predictions;
        }
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => navigation.navigate('ExpertPredictionDetail', { predictionId: item.id })}
    >
...
export default ExpertPredictionListScreen;
